...
headOr ::
  ...
  -> List a
  -> a
--headOr def (x:_) = if x then x else def
headOr def Nil = def
headOr _  (h :. _) = h
...
seqOptional ::
  ...
  List (Optional a)
  -> Optional (List a)
seqOptional lst = if any (== Empty) lst then Empty else Full $ map (\ (Full a) -> a ) lst
